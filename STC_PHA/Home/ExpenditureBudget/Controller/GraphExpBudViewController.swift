...
class GraphExpBudViewController: BaseViewController, ChartViewDelegate {
    ...
    func setupChart() {
        for i in 0..<sharedInstance.arrExpBud.count {
            times.append(sharedInstance.arrExpBud[i].name!)
            units.append(sharedInstance.arrExpBud[i].expBudget!.double)
            units1.append(sharedInstance.arrExpBud[i].lost!.double)
