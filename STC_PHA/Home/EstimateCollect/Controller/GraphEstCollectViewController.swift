...
class GraphEstCollectViewController: BaseViewController, ChartViewDelegate {
    ...
    func setupChart() {
        for i in 0..<sharedInstance.arrEstCol.count {
            times.append(sharedInstance.arrEstCol[i].name!)
            units.append(Double(sharedInstance.arrEstCol[i].money!)! / Double(Constant.currencyNumber))
        }
