//
//  GraphEstCollectViewController.swift
//  STC_PHA
//
...
class GraphEstCollectViewController: BaseViewController, ChartViewDelegate {
    
    var sharedInstance: GraphEstCollectViewController {
        struct Static {
            static let instance = GraphEstCollectViewController()
        }
    ...
    func initData() {
        ...
        let param = ["SHKB": curUser.maDBHC, "FROM_THANG":getTuThang()!, "TO_THANG":getDenThang()!, "NAM": getNam()!, "DONVITINH": 1, "LOAIQT": "T"] as Parameters
    ...
    func setupChart() {
        for i in 0..<arrEstCol.count {
            times.append(arrEstCol[i].name!)
            units.append(arrEstCol[i].money!)
        }
